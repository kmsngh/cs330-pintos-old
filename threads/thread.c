...
void thread_unblock(struct thread *t)
{
	...
	if (curr_thread != idle_thread && t->priority > curr_thread->priority)
	{
		t->status = THREAD_READY;
		curr_thread->status = THREAD_READY;
		list_insert_ordered(&ready_list, &curr_thread->elem, priority_high, NULL);
...
{
	struct thread *highest_ready_thread;
	struct thread *curr_thread;
	...
	highest_ready_thread = list_entry(list_begin(&ready_list), struct thread, elem);

	if (new_priority < highest_ready_thread->priority)
	{
		curr_thread->status = THREAD_READY;
		list_insert_ordered(&ready_list, &curr_thread->elem, priority_high, NULL);
		schedule();
